...
export const loadBlogPosts = async function* () {
  ...
  const dirPath = path.resolve(process.cwd(), 'data', 'blog', 'en');
...
export const publishBlogPostAssets = async (id: string) => {
  const dirPath = path.resolve(process.cwd(), 'data', 'blog', 'en', id);
  const targetDirPath = path.resolve(process.cwd(), 'public', 'blog', 'en', id);
...
export const publishBlogFeed = async () => {
  ...
  for await (const post of loadBlogPosts()) {
    feed.addItem({
      id: getSiteUrl(`/blog/en/${post.id}`),
      link: getSiteUrl(`/blog/en/${post.id}`),
